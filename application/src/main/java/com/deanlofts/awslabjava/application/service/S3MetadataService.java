...
public class S3MetadataService {
    ...
    public void writeWidgetMetadata(Widget widget) {
        ...
        try {
            ...
            String payload = objectMapper.writeValueAsString(new WidgetMetadata(widget, Instant.now(), context.deploymentTarget()));
    ...
    private S3Context resolveContext() {
        Assert.isTrue(appProperties.getFeature().isS3Metadata(), "S3 metadata feature flag must be enabled");
        String bucketName = awsProperties.getS3().getBucketName();
        if (!StringUtils.hasText(bucketName)) {
            throw new IllegalStateException("S3 metadata enabled but aws.s3.bucket-name is not configured");
        }
        S3Client client = s3Client.orElseThrow(() -> new IllegalStateException("S3 metadata enabled but S3 client is not available"));
    ...
    private record S3Context(S3Client client, String bucketName, String prefix, String deploymentTarget) {
